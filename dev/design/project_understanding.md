...
The two `.md` guides are early, informal notes; superseded by `dev/design/project_understanding.md` (this file) and the rest of the new `docs/` tree.
...
2. **Notebook → package migration.** The two `robstattm/python/*.ipynb` notebooks contain ad-hoc wrapper code. We will extract the patterns but not import them; the installable package gets fresh, documented code per the standards in `dev/design/documentation_standards.md`.
...
- Per-function research: → `the vendored R source (see dev/design/RELOCATED.md)`
- Dependency graph: → `dev/design/dependency_map.md`
- Architecture: → `dev/design/architecture.md`
- Validation policy: → `dev/design/validation_strategy.md`
...
- Documentation standards: → `dev/design/documentation_standards.md`
- Implementation rules + quality gates: → `dev/design/implementation_rules.md`, `dev/design/quality_gates.md`
