# dev/ - manual scratch and inspection scripts
...
The per-family smoke scripts that used to live here (`_smoke_check.py`,
`_smoke_dcml.py`, and friends) have been consolidated into the repo-root
`verify.py`, which runs every estimator family end-to-end and prints
human-shaped output:

```bash
python verify.py --quick      # smoke only (~5s)
python verify.py              # smoke + full pytest suite
```

Use that instead when you want to watch a fit rather than assert on it.
