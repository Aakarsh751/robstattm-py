...
| **Smoke scripts** | `tests/_smoke_*.py` | ad hoc | Manual one-off checks (run with `python`) |
...
```bash
python tests/_smoke_check.py          # environment
python tests/_smoke_lmrobdet.py       # MM regression
python tests/_smoke_dcml.py           # DCML + pyinit
python tests/_smoke_pense.py          # pense / pense_cv
python tests/_smoke_step_rlt.py       # stepwise + linear test
python tests/_ui_demo.py              # UI ergonomics tour
